...
You need Go (the version in `.go-version`) and `make`. Pinned linters and
scanners install themselves into `bin/tools` on first use, so there is nothing
to install by hand. **You do not need a DevRadar account, and you do not need
any credentials from the maintainer** — the test suite runs entirely offline.
If you hit a step that seems to require either, that is a bug; please file it.

```shell
git clone https://github.com/thingzio/devradarctl && cd devradarctl
make test       # unit tests with the race detector
make lint       # go vet + golangci-lint
make license    # stamp the Apache-2.0 header on any new Go file
make qualify    # the full gate CI runs
```

New Go files need the Apache-2.0 header. `make license` adds it; `make qualify`
fails without it, so you will not find out at review time.

`make qualify` is the same gate CI runs. If it passes locally it should pass in
CI; if it does not, that is worth an issue.

See [DEVELOPMENT.md](DEVELOPMENT.md) for the project layout, the API contract,
and the full list of `make` targets.
...
Open an [issue](https://github.com/thingzio/devradarctl/issues/new). The most
useful bug report contains the smallest reproduction you can manage — a failing
test is ideal, a command with its output is good, a description of the behavior
is workable. Redact tokens before pasting output.

### Suggesting enhancements

Lead with the problem you are trying to solve rather than the solution you have
in mind; the problem statement is the part most likely to change the design.
...
#### The service API is a contract, not an implementation detail

`internal/client` is validated against the DevRadar service's published OpenAPI
document, a copy of which is vendored at `internal/client/testdata/openapi.yaml`.
A change to the request or response shape belongs with a refreshed copy of that
spec — see [DEVELOPMENT.md](DEVELOPMENT.md#api-contract). If
`TestOpenAPISpec_IsCurrent` fails, the spec moved; that is a signal to look, not
to edit the test.

## Pull request process

1. **Make `make qualify` pass.** Format check, license headers, tests with the
   race detector and the coverage floor, golangci-lint, actionlint, a secret
   scan, and a vulnerability scan.
...
- Wrap errors with context: `fmt.Errorf("context: %w", err)`, never a bare
  `return err`
- Log with `log/slog`, never `fmt.Println`. The default level is `warn`: a CLI
  that is chatty by default is a CLI people pipe to `/dev/null`
...
- Every outbound HTTP call uses a client with a timeout, never
  `http.DefaultClient`
- **A secret is never a flag.** Tokens come from the environment or from stdin;
  process arguments are readable by every other process on the host
...
Open an [issue](https://github.com/thingzio/devradarctl/issues/new). Search
